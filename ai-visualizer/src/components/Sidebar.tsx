...
import { LayoutDashboard, ShieldAlert, Zap ,FileCode} from "lucide-react";
import { cn } from "@/lib/utils";

const menuItems = [
  { icon: LayoutDashboard, label: "Dashboard", href: "/" },
  { icon: ShieldAlert, label: "Spam Detection", href: "/spam-detection" },
  { icon: FileCode, label: "Churn Prediction", href: "/churn" },
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className="block group">
              <div
                className={cn(
                  "flex items-center gap-3 px-4 py-4 rounded-xl transition-all duration-300 relative overflow-hidden",
                  isActive
